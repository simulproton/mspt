...
int calcRadiolDepthDouble(VOLUME *stpPwr_grid,VOLUME *radiological_depth_grid,
    ...
    double xSpacing = (stpPwr_grid->inc).x;
    double ySpacing = (stpPwr_grid->inc).y;
    double zSpacing = (stpPwr_grid->inc).z;
    ...
    double startX = (stpPwr_grid->start).x;
    double startY = (stpPwr_grid->start).y;
    double startZ = (stpPwr_grid->start).z;
...
double coordPlane(int idx, double coordPlane1, double spacing){
    /*
     Function that comptes and return the coordinate of a plane for a given index (idx)
...
int computeAlpha(int idx, double coord1, double coord2,double coordPlane1, double spacing , double * ptAlpha){
    /*
     Function that compute alpha value for a given index (idx).
     For instance:
     if X2 - X1 != 0:
    ...
    Note: for undefined alpha values we will set alpha to DBL_MAX, and return -1,
    ...
    */
    ...
    double deltaCoord = coord2 - coord1;
//     printf("deltaCoord: %f = %f - %f \n",deltaCoord,coord2,coord1);
    if (absolute(deltaCoord) <= 1e-5){
        (*ptAlpha) = DBL_MAX;
        return -1;
    }
    double coordPl = coordPlane(idx,coordPlane1,spacing);
...
void sortAlphaMinMax(double * alphasMinMax){
    /*
    Function that make sure that alpha min is the smallest value et alpha max the highest
    */
    
    double min = alphasMinMax[0];
    double max = alphasMinMax[1];
    if (max < min){
        alphasMinMax[1] = min;
        alphasMinMax[0] = max;
    }
...
void findAlphaMinMax(double * alphasX, double * alphasY, double * alphasZ, double * alphaMinMax){
    /*
     Function that find alpha_min and alpha_max from alpha values. See eq(5) in Siddon 1985.
    ...
    */
    ...
    double tabMax[4];
    tabMin[0] = 0;
    tabMin[1] = minX == DBL_MAX ? 0 : minX;
    tabMin[2] = minY == DBL_MAX ? 0 : minY;
    tabMin[3] = minZ == DBL_MAX ? 0 : minZ;
    
    tabMax[0] = 1;
    tabMax[1] = maxX == DBL_MAX ? 1 : maxX;
    tabMax[2] = maxY == DBL_MAX ? 1 : maxY;
    tabMax[3] = maxZ == DBL_MAX ? 1 : maxZ;
...
void findIndMinMax(double coordPlane1,double coordPlaneN,double spacing,double coord1, double coord2, int nbPlanes, double * alphaMinMax , int * indMinMax){
    /* Find minimum index of intersected planes for given coordinate planes.
    ...
    double tmpVal;
    if(coord2 - coord1 >= 0){
        double valNbPlanes = nbPlanes;
        tmpVal = valNbPlanes - (coordPlaneN - alphaMinMax[0] * (coord2 - coord1) - coord1) / spacing ;
        ...
        tmpVal = nbPlanes - ((coordPlaneN - alphaMinMax[1] * (coord2 - coord1) - coord1) / spacing);
        indMinMax[0] = roundFloat(tmpVal);
...
void mergeSets(double * alphaMinMax, double * alphaX, int lenAlphaX, double * alphaY, int lenAlphaY , double * alphaZ, int lenAlphaZ, double * alpha, double d12 , double spacing){
    /*
     ...
     alpha: table to fill with alpha values merged: it should be allocated with (lenAlphaX+lenAlphaY+lenAlphaZ+2) double values
    ...
    if (alphaX[0] == DBL_MAX) lenAlphaX = 0;
    if (alphaY[0] == DBL_MAX) lenAlphaY = 0;
    if (alphaZ[0] == DBL_MAX) lenAlphaZ = 0;
        
    double * tab0 = (double *) malloc(sizeof(double) * (lenAlphaX + lenAlphaY));
    if(!tab0){
        perror("Error - memory allocation tab0 in merge alphas\n");
        exit(1);
    }
    double * tab1 = (double *) malloc(sizeof(double) * (lenAlphaX + lenAlphaY+lenAlphaZ) );
    if(!tab1){
...
void computeVoxelsLengths(double * alpha, int lenAlpha, double * lengthsVoxels, double d12){ // can be skipped and done directly when computing the radiological path
    /*
     ...
     lengthsVoxels: table where lengths will be stored. It should be allocated with a (lenAlpha-1) double values
...
/* 
    ...
    */ 
    ...
    double plane1x = plane1.x;
    double plane1y = plane1.y;
    double plane1z = plane1.z;
    
    double spacingx = spacing.x;
    double spacingy = spacing.y;
    double spacingz = spacing.z;
    

    double alphaMid, tmpVal;
...
int roundFloat( double value){
    ...
    double tmp = floor((double)value);
    int ret;
    if ( ((double)value - tmp) > 0.5)
		ret = (int) tmp + 1 ; 
	else
		ret = (int) tmp ;
    return ret;
            ...
            if (listInd[m-1][0] > 0 && listInd[m-1][1] > 0 && listInd[m-1][2] > 0 ){
                ...
                if ( i < nbCols && j < nbRows && k < nbFrames){ 
                    ...
                    if(voxelDensity < 0){
                        ...
                        perror("In cal radiol depth : density < 0!\n");
                        return -1;
                    }
