...
int singleRaytrace(VOLUME * grid,
    ...
    float xSpacing = (grid->inc).x;
    float ySpacing = (grid->inc).y;
    float zSpacing = (grid->inc).z;
    
    int iMinMax[2];
    int jMinMax[2];
    int kMinMax[2];
    
    float startX = (grid->start).x;
    float startY = (grid->start).y;
    float startZ = (grid->start).z;
    ...
    (*indRay).lenVoxels = (float *)malloc(sizeof(float) * (*indRay).nbVoxels);
    if(!( (*indRay).lenVoxels )){
        perror("Error - memory allocation list dist\n '(*indRay).lenVoxels = (float *)malloc(sizeof(float) * (*lenVoxels).nbVoxels)'\n");
        return FAILURE;
    } 
