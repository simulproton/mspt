...
#include "fillCTArray_PythonExt.h"
...
extern "C" { 
    void init_fillCTArray_PythonExt()  {
        (void) Py_InitModule("_fillCTArray_PythonExt", _CPP_extensionMethods);
 ...
 */ 
    ...
    float  * dataNewCTGrid, * dataCTGrid;
    ...
    PyArrayObject * newCTGrid = NULL;
       // create output grid
    if(  ( newCTGrid = (PyArrayObject *) PyArray_FromDims(ctGrid->nd, dimGrid, PyArray_FLOAT) ) == NULL ){
        PyErr_SetString(PyExc_ValueError,"Error - malloc in fillCTGrid - newCTGrid allocation failed\n");
    }
    dataNewCTGrid = (float*) newCTGrid -> data;
