...
// Module name must be _singleRay_PythonExt to compile and link
extern "C" { 
    void init_singleRay_PythonExt()  {
        (void) Py_InitModule("_singleRay_PythonExt", _CPP_extensionMethods);
...
static PyObject * singleRayTrace(PyObject *self, PyObject *args){
    ...
    */
    ...
    float *gridPtr,*startPtr,*incPtr, *sourceVecPtr, *targetVecPtr;
    float *indicesPtr;
    int dimList[2];
    ...
    // other
    int numDimGrid;//Number of dimensions of the grid
    ...
    // Calculate voxel indices along the ray
    if (singleRaytrace(&gridValues,p1,p2,&indices) == FAILURE)
        PyErr_SetString(PyExc_ValueError,errstr);
