...
########################################################################
#
# calcRadiolDepth_PythonExtDouble.cpp
# 
...
// Code based on "calcDeffParallel.cpp" provided by Ryan Flynn - University of Iowa, Iowa, USA
#include "calcRadiolDepth_PythonExtDouble.h"
...
// Module name must be _calcRadiolDepth_PythonExtDouble in compile and linked
extern "C" { 
    void init_calcRadiolDepth_PythonExtDouble()  {
        (void) Py_InitModule("_calcRadiolDepth_PythonExtDouble", _CPP_extensionMethods);
...
static PyObject * calcRadDeff(PyObject *self, PyObject *args){
    ...
    */
    ...
    double *densPtr,*startPtr,*incPtr,*deffPtr, *sourceVecPtr ;
    ...
    densPtr = (double *) densityGrid -> data;
    ...
    // create output grid
    if(  ( deffGrid = (PyArrayObject *) PyArray_FromDims(densityGrid -> nd, densDim, PyArray_DOUBLE) ) == NULL ){
            ...
            for (i=0;i<M;i++){
                if (deff.matrix[index] == -1.0)
                {
                    ...
                    // do the raytrace, filling in voxels that are passed on the way
                     if ( calcRadiolDepthDouble(&dens,&deff,p1,p2,countPtr) == FAILURE)
                         PyErr_SetString(PyExc_ValueError,errstr);
